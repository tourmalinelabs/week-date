...
describe('week-date', () => {
  describe('date', () => {
    ...
    it('should allow for year-week string', () => {
      expect(weekDate.date('2005-53').format('YYYY-MM-DD')).to.equal(
        '2006-01-07'
      );
    });
  });
  describe('week', () => {
    ...
    it('should allow for milliseconds since epoch', () => {
      expect(
        weekDate.week(moment.utc('2018-01-07').unix() * 1000)
      ).to.deep.equal({
        year: 2018,
        week: 1,
      });
  ...
  describe('adjustDate', () => {
    it('should adjust date to that weeks saturday', () => {
      expect(
        weekDate.adjustDate(moment.utc('2018-01-07')).format('YYYY-MM-DD')
